...
public class MusicPlayer {
	...
	private void updatePlayerSongListsColor(Table table, int index) {
		...
		if (Constant.MUSIC_PLAYER_SONGS_LIST.get(Constant.PLAYING_SONG_INDEX).split(Constant.MUSIC_PLAYER_SYSTEM_SPLIT)[4].equalsIgnoreCase("Y") && Constant.MUSIC_PLAYER_SYSTEM_START_LYRIC) {
	...
	public static void JVMinfo() {
		...
		ThreadGroup groups = Thread.currentThread().getThreadGroup();  
		ThreadGroup group = groups;
		while (groups != null) {
			group = groups;
			groups = groups.getParent();  
		}
		Thread[] slack = new Thread[group.activeCount() * 2];
		group.enumerate(slack);
		Stream.of(slack).forEach(t->{if (t !=null && t.getName().startsWith("Thread")) {
